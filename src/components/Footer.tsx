  ...
  return (
    <footer className="border-t border-[var(--site-border)] bg-slate-950 py-12 text-slate-100">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-5 text-center sm:px-6 md:grid-cols-3 md:text-left">
        {/* Contact & Socials */}
        <div>
          <h3 className="mb-4 font-bold text-cyan-300">{isAm ? 'አግኙኝ' : 'Get In Touch'}</h3>
          <p className="mb-6 text-sm text-slate-300/80">
            {isAm
              ? 'በTelegram፣ Facebook እና X ላይ ያግኙኝ፤ ወይም በኢሜይል ቀጥታ ይጻፉልኝ።'
        ...
        {/* Accomplishments */}
        <div>
          <h3 className="mb-4 font-bold text-cyan-300">{isAm ? 'ስኬቶች' : 'Accomplishments'}</h3>
          <p className="mb-4 text-sm text-slate-300/80">
            {isAm
              ? 'በተለያዩ የትምህርት እና የፈተና መድረኮች ላይ ያሉ ስኬቶቼን፣ ሰርቲፊኬቶቼን እና የኮዲንግ ተግባራትን ይመልከቱ።'
          ...
          <div className="flex flex-wrap justify-center gap-2 text-2xl md:justify-start">
            ...
            <a href="https://www.kaggle.com/asresgamu" target="_blank" rel="noreferrer" aria-label="Kaggle" title="Kaggle" className="rounded-lg p-2"><SiKaggle className="hover:text-blue-400 transition-colors" /></a>
            <a href="https://posit.cloud/spaces/736077/users/3504229" target="_blank" rel="noreferrer" aria-label="R / Posit Cloud" title="R / Posit Cloud" className="rounded-lg p-2"><SiR className="hover:text-blue-500 transition-colors" /></a>
        ...
        <div>
          <h3 className="mb-4 font-bold text-cyan-300">{isAm ? 'ቱቶሪያሎች እና ቴክ ስታክ' : 'Tutorials & Stack'}</h3>
          <p className="mb-4 text-sm text-slate-300/80">
            {isAm
              ? 'ለቱቶሪያሎች፣ የፕሮጀክት ማብራሪያዎች እና ተግባራዊ የዳታ ትንታኔ ይዘት ይከተሉኝ።'
      ...
      <div className="mt-12 text-center text-xs text-slate-300/50">
        {isAm
          ? `© ${new Date().getFullYear()} EthioCodingRoom. መብቶች ሁሉ የተጠበቁ ናቸው።`
